...
#ifdef USE_SEQ_CST
#define MO_acquire std::memory_order_seq_cst
#define MO_release std::memory_order_seq_cst
#define MO_relaxed std::memory_order_seq_cst
#else
...
namespace twoplsf {
  ...
  template<typename T> 
  void Retire(T* ptr) { epoch::memory_pool_<T>::Retire(static_cast<T*>(ptr)); }

  template <typename T, typename... Args>
  T* New(Args&&... args) { return epoch::memory_pool_<T>::New(args...); }

  template <typename R, typename F>
  R with_epoch(const F& f) { return epoch::with_epoch(f); }

  template <typename F>
  void with_epoch(const F& f) { epoch::with_epoch(f); }
#else
  template<typename T> 
  void Retire(T* ptr) { delete ptr;}

  template <typename T, typename... Args>
  T* New(Args&&... args) { return new T(args...); }

  template <typename R, typename F>
  R with_epoch(const F& f) { return f(); }

  template <typename F>
  void with_epoch(const F& f) { f(); }
#endif
...
public:
    ThreadRegistry() {
        for (int it = 0; it < REGISTRY_MAX_THREADS; it++) {
            usedTID[it].store(false, MO_relaxed);
        }
    }
    ...
    int __attribute__ ((noinline)) register_thread_new(void) {
        for (int tid = 0; tid < REGISTRY_MAX_THREADS; tid++) {
            if (usedTID[tid].load(MO_acquire)) continue;
            bool unused = false;
    ...
    inline void deregister_thread(const int tid) {
        usedTID[tid].store(false, MO_release);
    }

    // Progress condition: wait-free population oblivious
    static inline uint64_t getMaxThreads(void) {
        return gThreadRegistry.maxTid.load(MO_acquire);
    }
...
// We reserve 16 bits for the tid of the write lock. Use 0 to represent UNLOCKED and the others are tid+1
  static const uint64_t UNLOCKED = (1UL << 16)-1;
  struct wlock {
    uint64_t state;
    static constexpr uint64_t mask = (1ul << 16) - 1;
    bool is_unlocked() {return (state & mask) == UNLOCKED;}
    uint64_t get_id() {return state & mask; }
    wlock lock(uint64_t tid) {
      //return wlock(tid); }
    return wlock(((state >> 16) << 16) + tid); }
    wlock unlock() {
      //return wlock(); }
     return wlock((((state >> 16) + 1) << 16) + UNLOCKED); }
    bool operator==(wlock x) {return x.state == state;}
    wlock() noexcept : state(UNLOCKED) {}
    wlock(uint64_t id) noexcept : state(id) {}
  };

  bool try_write_lock(std::atomic<wlock>& l, uint64_t id) {
    wlock oldl = l.load(MO_acquire);
    wlock newl = oldl.lock(id);
    return oldl.is_unlocked() && l.compare_exchange_strong(oldl, newl);
  }

  void write_unlock(std::atomic<wlock>& l) {
    l.store(l.load(MO_acquire).unlock(), MO_release);
  }
...
// 2^5 => one lock for every 32 bytes (half a cache line)
  ...
  //inline static uint64_t addr2writeIdx(const void* addr) { return (((uint64_t)(addr) * 79) & (NUM_RWL-1)); }
...
struct ReadSet {
    ...
    static const int64_t MAX_READ_SET_ENTRIES = 128*1024;
...
struct WriteSet {
  ...
  inline void addEntry(const void* addr, uint64_t new_val) {
        assert(size != MAX_WRITE_SET_ENTRIES);
        entries[size].addr = (void*)addr;
#ifdef WRITE_BACK // log new value to write later on success
        filter.insert((unsigned long) addr);
        entries[size].data = new_val;
#else // log old value for undo on abort
  ...
  int find(const void* addr) {
    if (filter.find((unsigned long) addr))
      for (int i = size - 1; i >= 0; i--) 
        if (entries[i].addr == addr) return i;
    return -1;
    }
...
struct OpData {
...
#ifdef SV_TLF
    ReadSet               lockSet;                     // The write-lock set
#endif
  ...
  Deletable                 flog[TX_MAX_RETIRES];        // List of retired objects during the transaction (owner thread only)
...
public:
    ...
    alignas(128) std::atomic<wlock>* wlocks;
    ...
    STM() {
        ...
        for (int i=0; i < REGISTRY_MAX_THREADS; i++) txnTS[i*CLPAD].store(NO_TIMESTAMP, MO_release);
        wlocks = new std::atomic<wlock>[NUM_RWL];
        for (uint64_t i = 0; i < NUM_RWL; i++) wlocks[i].store(wlock(), MO_relaxed);
        readIndicators = new std::atomic<uint64_t>[NUM_RI_WORDS];
        for (uint64_t i = 0; i < NUM_RI_WORDS; i++) readIndicators[i].store(0, MO_relaxed);
    }
    ...
    double percentAborts() {
      ...
      return 100.0 * totalAborts/(1+totalCommits);
    }
    ...
    inline void endTx(OpData* myd, const int tid) {
...
#ifdef DELAYED_WRITE_LOCK
        for (uint64_t i=0; i < myd->writeSet.size; i++)
          if (!tryWaitWriteLock_(myd, myd->writeSet.entries[i].addr)) {
            myd->writeSet.size = i;
            abortTx(myd);
        ...
        // Write back values
        for (uint64_t i=0; i < myd->writeSet.size; i++) {
          *(uint64_t*)(myd->writeSet.entries[i].addr) = myd->writeSet.entries[i].data;
        }
#endif

        // Unlock write locks
#ifdef SV_TLF
        for (uint64_t i=0; i < myd->lockSet.size; i++)
          unlockWrite_(myd->lockSet.entries[i].widx, tid);
#else
        for (uint64_t i=0; i < myd->writeSet.size; i++)
          unlockWrite(myd->writeSet.entries[i].addr, tid);
#endif

        // Unlock the read locks
        unlockAllReadLocks(myd, tid);
        // Execute de-allocations
        for (uint64_t i = 0; i < myd->numFrees; i++) myd->flog[i].reclaim(myd->flog[i].obj);
        myd->numCommits++;
        myd->attempt = 0;
        // Clear the published timestamp for this thread
        txnTS[tid*CLPAD].store(NO_TIMESTAMP, MO_release);
  ...
  inline void abortTx(OpData* myd, bool enableRollback=true) {
        ...
        // Unlock write-locks
#ifdef SV_TLF
        for (int i=0; i < myd->lockSet.size; i++)
          unlockWrite_(myd->lockSet.entries[i].widx, myd->tid);
#else
        for (int i=0; i < myd->writeSet.size; i++)
          unlockWrite(myd->writeSet.entries[i].addr, myd->tid);
#endif
        // Unlock the read locks
  ...
  static T* tmNew(Args&&... args) {
    ...
    if (myd != nullptr) {
      ...
      del.obj = ptr;
      del.reclaim = [](void* obj) { Retire(static_cast<T*>(obj)); };
    } 
    return ptr;
  }
    ...
    template<typename T> static void tmDelete(T* obj) {
        ...
        Deletable& del = myopd->flog[myopd->numFrees++];
        del.obj = (tmbase*)obj;
        del.reclaim = [](void* obj) { Retire(static_cast<T*>(obj)); };
      }
    ...
    enum rlock_result : char {Own, Succeeded, Failed};                                                    
                                                                                                          
    inline bool tryWaitReadLock(OpData* myd, const void* addr) {                                          
      rlock_result r = tryWaitReadLock_(myd, addr);                                                       
    ...
    inline rlock_result tryWaitReadLock_(OpData* myd, const void* addr) {
        ...
        wlock wstate = wlocks[widx].load(MO_acquire);
        if (wstate.is_unlocked() || wstate.get_id() == myd->tid) return Succeeded;
        // Looks like there is a writer holding this lock. Enter slow-path
        return tryWaitReadLockSlowPath(myd, widx, ridx, newri);
    }


  inline bool tryWaitWriteLock(OpData* myd, const void* addr, uint64_t new_val) {
    bool ok = tryWaitWriteLock_(myd, addr);
    if (ok) myd->writeSet.addEntry(addr, new_val);
    return ok;
  }
  ...
  inline bool tryWaitWriteLock_(OpData* myd, const void* addr) {
    uint32_t widx = addr2writeIdx(addr);
    wlock wstate = wlocks[widx].load(MO_acquire);
    // Check if it's already write-locked by me, or if can take the lock
    if ((wstate.get_id() == myd->tid) ||
        (wstate.is_unlocked() &&
         try_write_lock(wlocks[widx], myd->tid) &&
         isEmpty(widx, myd->tid))) {
      return true;
    }
    ...
    inline void unlockWrite(const void *addr, uint16_t tid) {
        ...
        wlock wstate = wlocks[widx].load(MO_relaxed);
        // If write-locked by me, unlock it
        if (wstate.get_id() == tid) write_unlock(wlocks[widx]);
    }

  inline void unlockWrite_(uint64_t widx, uint16_t tid) {
        wlock wstate = wlocks[widx].load(MO_relaxed);
        // If write-locked by me, unlock it
        if (wstate.get_id() == tid) write_unlock(wlocks[widx]);
    }
    ...
    inline void unlockRead(uint32_t widx, uint16_t tid) {
        ...
        readIndicators[ridx].store(ri & (~rmask), MO_release);
    }
...
private:
    ...
    inline bool isEmpty(uint32_t widx, uint32_t tid) {
        ...
        for (uint32_t itid = 0; itid < maxThreads; itid++) {
            ...
            uint64_t ri = readIndicators[ridx].load(MO_acquire);
    ...
    rlock_result __attribute__ ((noinline)) tryWaitReadLockSlowPath(OpData* myd, uint32_t widx, uint32_t ridx, uint64_t ri) {
        ...
        if (txnTS[myd->tid*CLPAD].load(MO_relaxed) == NO_TIMESTAMP) txnTS[myd->tid*CLPAD].exchange(myd->myTS);
        while (true) {
            // Check the writer's cohort lock state
            if (wlocks[widx].load(MO_acquire).is_unlocked()) {
                txnTS[myd->tid*CLPAD].store(NO_TIMESTAMP, MO_release);
                return Succeeded;
            }
            ...
            if (myd->oTS < myd->myTS) {
                ...
                readIndicators[ridx].store(ri & (~ribit(widx)), MO_release);
                return Failed;
            }
    ...
    bool __attribute__ ((noinline)) tryWaitWriteLockSlowPath(OpData* myd, uint32_t widx) {
        ...
        while (true) {
            ...
            wlock wstate = wlocks[widx].load(MO_acquire);
            if (wstate.is_unlocked()) try_write_lock(wlocks[widx], myd->tid);
            wstate = wlocks[widx].load(MO_acquire);
            if (wstate.get_id() == myd->tid && isEmpty(widx, myd->tid)) {
            ...
            if (myd->oTS < myd->myTS) {
                ...
                if (wlocks[widx].load().get_id() == myd->tid) write_unlock(wlocks[widx]);
                return false;
            }
            // We're in "Wait" mode for now
            Pause();
    ...
    inline uint64_t getTSOfWLock(uint32_t widx, uint64_t tid, uint16_t& lowestTid) {
        ...
        wlock wstate = wlocks[widx].load(MO_acquire);
        if (!wstate.is_unlocked() && wstate.get_id() != tid) {
          uint64_t otid = wstate.get_id();
...
template<typename T> struct tmtype {
  ...
  inline void pstore(T newVal) {
    ...
    if (myd == nullptr)
      val = newVal;
#if defined(DELAYED_WRITE_LOCK) || defined(SV_TLF)
    else myd->writeSet.addEntry(&val, (uint64_t) newVal);
#else
    else if (gSTM.tryWaitWriteLock(myd, &val, (uint64_t) newVal)) {
#ifndef WRITE_BACK
      val = newVal;
#endif
    } else abortTx(myd);
  ...
  inline T pload() const {
    ...
    return pload_internal(myd);
#else
    return pload_internal<false>(myd);
#endif
  }

  inline T pload_weak() const {
    OpData* const myd = tl_opdata;
    if (myd == nullptr) return val;
    return pload_internal(myd);
  }
...
#ifdef Committed
    template <bool committed=true>
#else
    template <bool committed=false>
#endif
    inline T pload_internal(OpData* myd) const {
#ifdef WRITE_BACK
      uint32_t widx = addr2writeIdx(&val);
      wlock wstate = gSTM.wlocks[widx].load(MO_acquire);
#if defined(DELAYED_WRITE_LOCK) || defined(SV_TLF)
      if (true) {
#else
        if (wstate.get_id() == myd->tid) {
#endif
        int i = myd->writeSet.find(&val);
        if (i != -1) return (T) myd->writeSet.entries[i].data;
      }

      // if a committed-read then cannot read while a transaction is
      // writing need to take a read lock while reading.
      if (committed) {
        T v = val;
        // a "sequence lock".  Checks that lock is unlocked and has
        // not changed from before reading val and after reading val
        if (wstate.is_unlocked() && wstate == gSTM.wlocks[widx].load(MO_acquire))
          return v;
        STM::rlock_result x = gSTM.tryWaitReadLock_(myd, &val);
        if (x == STM::Own) return val;
        else if (x == STM::Succeeded) {
          T v = val;
          gSTM.unlockRead(widx, myd->tid);
          return v;
        } else abortTx(myd);
      }
#endif
    ...
    inline void validate(T tval) const {
      OpData* const myd = tl_opdata;
      if (myd == nullptr) return;
      T x = pload();
      if (x != tval) abortTx(myd);
    }
};
...
void thread_registry_deregister_thread(const int tid) {
  ...
  [[noreturn]] void __attribute__ ((noinline)) abortTx(OpData* myd) {
    gSTM.abortTx(myd, true);
    std::longjmp(myd->env, 1);
  }

#ifdef SV_TLF
  void try_read_lock(const void* addr) {
    auto myd = tl_opdata;
    if (!gSTM.tryWaitReadLock(myd, addr)) {
      abortTx(myd);
    }
  }

  void try_write_lock(const void* addr) {
    auto myd = tl_opdata;
    if (!gSTM.tryWaitWriteLock_(myd, addr)) {
      twoplsf::abortTx(myd);
    }
    myd->lockSet.addEntry(addr);
  }
#endif

  double percentAborts() { return gSTM.percentAborts(); }
