...
struct ordered_map {
  ...
  constexpr static int max_sparse_size = (sizeof(K) + sizeof(V) > 16) ? 16 : 32;
  static constexpr int max_small_leaf_size = 2;
  static constexpr int max_big_leaf_size = (sizeof(K) + sizeof(V) > 16) ? 14 : 30;
  ...
  struct alignas(64) generic_leaf : header {
    ...
    static void update(KV* in, KV* out, int n, const K& key, const V& value,
      ...
      int i = 0;
      while (i < n && less(in[i].key, key, byte_pos)) { 
        out[i] = in[i];
        i++;
      }
      out[i] = KV{key, value};
  ...
  static void add_child(node* gp, node* p, const K& k, const V& v) {
  ...
  static auto find_location(node* root, const K& k, bool use_weak = false) {
    int byte_pos = 0;
    node* gp = nullptr;
    node* p = root;
    while (true) {
      ...
      node* c = use_weak ? cptr->load_weak() : cptr->load();
      if (c == nullptr) // has empty child with given key
        return std::make_tuple(gp, p, cptr, c, byte_pos);

      // check how many bytes of k match c->key
      byte_pos++;
      while (byte_pos < c->byte_num &&
             String::get_byte(k, byte_pos) == String::get_byte(c->key, byte_pos))
        byte_pos++;

      // Return if either c->key only partially matches k (i.e. k is
      // not in the tree), or c is a leaf.  Note that bype_pos is now
      // set to the number of bytes that match.
  ...
  bool insert_(const K& k, const V& v) { return try_insert(k, v); }
  bool insert(const K& k, const V& v) { return try_insert(k, v); }
  
  bool upsert_(const K& k, const V& v) { return try_insert(k, v, true);}
  bool upsert(const K& k, const V& v) { return try_insert(k, v, true);}

  template <typename F>
  bool upsert_f(const K& k, const F& f) { return upsert_(k,f(find_(k))); }

  bool try_insert(const K& k, const V& v, bool upsert = false) {
  ...
  bool remove_(const K& k) { return try_remove(k); }
  bool remove(const K& k) {  return try_remove(k); }

  // currently a "lazy" remove that only removes
  //   1) the leaf
  //   2) its parent if it is sparse with just two children
  bool try_remove(const K& k) {
    auto [gp, p, cptr, c, byte_pos] = find_location(root, k);
    //cptr->validate();
    //p->removed.validate();
  ...
  std::optional<V> find_(const K& k) {
    auto [gp, p, cptr, l, pos] = find_location(root, k, true);
    if (cptr != nullptr) cptr->validate(l);
    p->removed.validate(false);
    if (l == nullptr) return {};
