...
struct ordered_map {
  ...
  // internal node
  struct alignas(64) internal : node {
    ...
    internal(K k, node* left, node* right)
      : node{k,false}, left(left), right(right) {};
    internal(node* left) // for the root, only has a left pointer
      : node{false}, left(left), right(nullptr) {};
  };
  ...
  static auto find_location(internal* root, const K& k) {
    ...
    while (!l->is_leaf) {
      ...
      p = (internal*) l;
      p_left = Less(k, p);
      l = p_left ? (p->left).load_weak() : (p->right).load_weak();
    }
    if (gp) {
      gp->removed.validate(false);
      if (gp_left) gp->left.validate(p);
      else gp->right.validate(p);
    }
    if (p_left) p->left.validate(l);
    else p->right.validate(l);
    return std::make_tuple(gp, gp_left, p, p_left, (leaf*) l);
  }
  
  bool upsert(const K& k, const V& v) {
    return verlib::with_epoch([=] { return insert_(k, v, true);});
  }
  ...
  std::optional<bool> try_remove(const K& k) {
    ...
    if (l->is_sentinal || !Equal(k, l)) return false;
    node* ll = (p->left).load();
